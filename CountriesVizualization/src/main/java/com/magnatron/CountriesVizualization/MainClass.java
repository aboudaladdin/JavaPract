...
import countriesdata.WorldCountriesDao;
import countriesdata.WorldCountriesDaoImp;
...
public class MainClass 
{
    public static void main( String[] args ) throws IOException
    {
		...
		List<Country> countries = worldDao.getAllCountries();
		new SwingWrapper<CategoryChart>(Scatter_x.getBarChart(countries)).displayChart();
		...
		while(!countryCode.equals("0"))
		{
	        ...
	        try
	        {
	        List<City> cities = worldDao.getSortedCities_by_CCode()
	        		.getOrDefault(countryCode.toUpperCase(), null);
			
	        new SwingWrapper<PieChart>(Scatter_x.getCityPieChart(cities)).displayChart();
